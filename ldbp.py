...
# avoid TF logging output for GPU devices etc.
...
def get_optimizer():
    if optimizer == "adam":
        opt = tf.train.AdamOptimizer(learning_rate, float(
            conf_train['adam_A']), float(conf_train['adam_B']))
    elif optimizer == "rmsprop":
        opt = tf.train.RMSPropOptimizer(learning_rate, float(
            conf_train['rmsprop_A']), float(conf_train['rmsprop_B']))
    elif optimizer == "adadelta":
        opt = tf.train.AdadeltaOptimizer(
            learning_rate, float(conf_train['adadelta_A']))
    elif optimizer == "adagrad":
        opt = tf.train.AdagradOptimizer(
            learning_rate, float(conf_train['adagrad_A']))
    else:
...
def cconv(x, h):
    ...
    if(filter_length % 2 == 0):  # even-length filters
        ...
        # [x_end, x, x_begin]
        x = tf.concat([x[:, N-filter_delay-1:, :], x,
                       x[:, :filter_delay, :]], axis=1)
    else:  # odd-length filters
        filter_delay = (filter_length-1)//2
        # [x_end, x, x_begin]
        x = tf.concat([x[:, N-filter_delay:, :], x,
                       x[:, :filter_delay, :]], axis=1)
    ...
    y = tf.nn.conv1d(x, conv1d_filter, stride=1,
                     padding="VALID", name='conv1d')
...
if load_cd_filter == True:
    ...
    cd_filter_oob_gain = conf_ldbp.getfloat(
        'cd filter max out-of-band gain')  # 0.58 for 17-taps 20 Gbaud
...
# batch size to pupulate the queue
QBSIZE = conf_data.getint('generation batch size')
# number of processors used to populate the queue
NPROC = conf_data.getint('number of parallel processors')
# replication factor for data
REPF = conf_data.getint('data replication factor')
...
if nl_filter_length % 2 == 0:
    raise ValueError(
        'nl_filter_length has to be odd: nl_filter_length = {}'.format(nl_filter_length))
...
print("total memory of the data queue: {} MB".format(
    QMAX*64*(Nsamp_d+Nsym)/8/1e6))
...
if args.timing == True:
    ...
    print(
        "Generating approx. {0:.0f} input/output data pairs per seconds".format(NPROC*REPF/elapsed))
    sys.exit("")
...
if load_cd_filter == False:
    ...
    # 0.58 for 17-taps 20 Gbaud
    fopt['max_out_of_band_gain'] = cd_filter_oob_gain
    ...
    if config.has_option('LDBP parameters', 'cd filter length'):
        ...
        print("No cd filter length provided. Computing automatically with margin {} and minimum {}:".format(
            cd_filter_length_margin, cd_filter_length_min))

        req_len = fir_obj.get_required_filter_length(bw.cd_length)
        cd_filter_length = (
            2*np.ceil(req_len/2*(1+cd_filter_length_margin))+1).astype(np.int32)
        for NN in range(bw.model_steps):
    ...
    # compute filter taps
    for NN in range(bw.model_steps):
        cd_filter_coeffs[NN] = fir_obj.get_filter(
            bw.cd_length[NN], cd_filter_length[NN])
    ...
    if len(lines) != 3*bw.model_steps:
        raise RuntimeError("File '"+cd_filter_filename +
                           "' should have {} lines but has {}".format(3*bw.model_steps, len(lines)))
    ...
    for NN in range(bw.model_steps):
        ...
        if np.size(h_r) != np.size(h_i):
            raise ValueError(
                'real and imaginary part of loaded cd filters should be the same')
        cd_filter_length[NN] = np.size(h_r)
        if cd_filter_length[NN] % 2 == 0:
            raise ValueError('loaded cd filter should have odd length: cd_filter_length[{}]={}'.format(
                NN, cd_filter_length[NN]))
...
def get_prune_op(mask, mask_len):
    ...
    pos = tf.constant(
        np.arange(0, int(mask.get_shape()[0]), 1, np.int32), tf.int32)
...
if pruning == True:
    ...
    pruning_schedule = (
        np.ceil(np.ceil(2.0**(-np.arange(pruning_steps, 0, -1))*iterations)))
    pruning_schedule = np.ceil(
        pruning_schedule + np.arange(pruning_steps)*iterations/8/pruning_steps)

    cd_mask_len = {}
    prune_op = {}  # each mask has a pruning op associated with it

    for NN in range(bw.model_steps):
        cd_mask[NN] = tf.Variable(
            np.ones([cd_filter_delay[NN]+1]), dtype=tf.float32, trainable=False)
        cd_mask_len[NN] = tf.Variable(
            cd_filter_delay[NN]+1, dtype=tf.int32, trainable=False)
...
for NN in range(bw.model_steps):
    # linear parameters
    cd_filter[NN] = tf_complex_symmetric_filter(
        cd_filter_coeffs[NN]*cd_alpha, cd_opt, mask=cd_mask[NN])
    # nonlinear parameters
    if nl_opt == True:
        if tied_Kerr == True:
            nl_filter[NN] = nl_filter_all
        else:
            nl_filter[NN] = tf_real_symmetric_filter(
                no_filter*nl_alpha, nl_opt)
    else:
...
# at least this many elements must remain after dequeue
min_after_dequeue = int(conf_data["minimum elements after dequeue"])

myq = tf.RandomShuffleQueue(QMAX, min_after_dequeue, dtypes=[
                            tf.float32, tf.complex64, tf.float32], shapes=[[Nsamp_d, 2], [Nsym], [1]])
...
for NN in range(bw.model_steps):
    ...
    # linear step
    # complex(y) = complex(x) * complex(h)
    y = cconv(y, cd_filter[NN]/cd_alpha)
    # nonlinear step, includes possible filtering of {|y_i|^2}
    #ysq = bw.nl_param[NN]*tf.reduce_sum(tf.square(y), axis=2)
    ysq = bw.nl_param[NN]*tf.reduce_sum(tf.square(y), axis=2)
    # real(y) = real(x) * real(h)
    ysq_filtered = cconv(ysq, nl_filter[NN]/nl_alpha)
    y = complex_multiply(y, tf.stack(
        [tf.cos(ysq_filtered), tf.sin(ysq_filtered)], axis=2))
...
def populate_queue(sess, enqueue_op, coord):
    ...
    while not coord.should_stop():
        ...
        sess.run(enqueue_op, feed_dict={
                 y_enq: y_batch, x_enq: x_batch, P_enq: P_batch})
...
print("---------------------------------------------")
print("initial: MSE = {0:.6f}, effective SNR = {1:.3f} dB".format(
    mse_tmp, snr_tmp))
...
for i in range(1, iterations+1):
    _, mse_tmp, snr_tmp, sstr = sess.run(
        [train, mean_squared_error, effective_snr, summary])  # 1 step in gradient descent
    ...
    if i % summary_interval == 0 or i == iterations:
        ...
        summary_writer.flush()
        print("iter {0}: MSE = {1:.6f}, effective SNR = {2:.3f} dB, summary written".format(
            i, mse_tmp, snr_tmp))
    # pruning
    if pruning == True:
        pr_cnt = (pruning_schedule == i).sum()
        for II in range(pr_cnt):
            print("pruning (iter: {}, filter: {}, progress: {}/{})".format(i,
                                                                           prune_order[pruned], pruned+1, pruning_steps))
...
print("processing approx. {0:.0f} input/output data pairs per second".format(
    iterations*minibatch_size/opt_time))
...
if SAVE_FILE == True:
    ...
    for NN in range(bw.model_steps):
        ...
        np.savetxt(f, np.transpose(
            nl_filter_print[NN]*bw.nl_param[NN] / nl_alpha), delimiter=',')
    f.close()
    print("done")
else:
